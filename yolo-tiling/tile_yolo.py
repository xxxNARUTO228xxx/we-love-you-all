...
if __name__ == "__main__":
    ...
    args_tuple = namedtuple("args", "source target ext falsefolder size ratio")
    args = args_tuple(
        source="datasets_processing/data/yolo",
        target="datasets_processing/data/yolosliced",
        ext=".jpg",
        falsefolder=None,
        size=256,
        ratio=0.8,
    ...
    if len(imnames) == 0:
        ...
        raise Exception(
            "Dataset should contain equal number of images and txt files with labels"
        )
    ...
    if not os.path.exists(os.path.join(upfolder, "classes.names")):
        ...
        copyfile(
            os.path.join(upfolder, "classes.names"),
            os.path.join(target_upfolder, "classes.names"),
        )
